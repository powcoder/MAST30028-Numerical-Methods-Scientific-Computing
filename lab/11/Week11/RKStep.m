...
elseif k==2
   k1 = h*fc;
   k2 = h*fname(tc+h,yc+k1);
   ynew  = yc + (k1 + k2)/2;
elseif k==3 
   k1 = h*fc;
   k2 = h*fname(tc+(h/2),yc+(k1/2));
   k3 = h*fname(tc+h,yc-k1+2*k2);
...
elseif k==5
   k1 = h*fc;
   k2 = h*fname(tc+(h/4),yc+(k1/4));   
   k3 = h*fname(tc+(3*h/8),yc+(3/32)*k1+(9/32)*k2);    
   k4 = h*fname(tc+(12/13)*h,yc+(1932/2197)*k1-(7200/2197)*k2+(7296/2197)*k3);               
   k5 = h*fname(tc+h,yc+(439/216)*k1 - 8*k2 + (3680/513)*k3 -(845/4104)*k4);
   k6 = h*fname(tc+(1/2)*h,yc-(8/27)*k1 + 2*k2 -(3544/2565)*k3 + (1859/4104)*k4 - (11/40)*k5);
   ynew  = yc + (16/135)*k1 + (6656/12825)*k3 + (28561/56430)*k4 - (9/50)*k5 + (2/55)*k6;
end
tnew = tc+h;
fnew = fname(tnew,ynew);